...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
                        description='Extract homography from a list of videos')
...
def match_frames(prev_frame, curr_frame, params):
    """
    Match frames and return the homography
    ONLY if the motion estimation passed a series of tests
    """
    scale = params['fScale']
    ...
    if len(kp1) < min_num_kps or len(kp2) < min_num_kps:
        print 'too few kps: textureless / blurring'
        return np.zeros((3,3))
    ...
    if len(mask) == 0:
        print 'degenerated homography'
        return np.zeros((3,3))

    # translational component
    vx, vy = np.abs(M[0,2]), np.abs(M[1,2])

    # moving too fast (violating homography assumption)
...
def process_vid_file(vid_file):
    ...
    return output_frame_list, output_homography


if __name__ == '__main__':
    dataset = sys.argv[1]
    hom_path = '/media/meerahahn/General_Datasets/' + dataset + '/homography/'
    vid_path = '/media/meerahahn/General_Datasets/' + dataset + '/videos/'
    videos = [vid for vid in os.listdir(vid_path)]

    #video = '/media/meerahahn/General_Datasets/EDGTEA/videos/OP01-R02-TurkeySandwich.mp4'
    for vid in videos:
        ...
        if ('.avi' in os.path.basename(video)) or ('.mp4' in os.path.basename(video)) \
            ...
            output_frame_list, output_homography = process_vid_file(video)
            #print len(output_homography)
            np.save(hom_path + vid[:-3] + 'npy' , output_homography)
        else:
